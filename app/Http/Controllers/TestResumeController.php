...
class TestResumeController extends Controller
{
    ...
    public function getResult($text)
    {
        ...
        foreach ($lines as $line) {
            ...
            $is_header = false;
            if (preg_match($noise_patterns, $line)) {
                continue;
            }
            ...
            if (preg_match('/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}/', $line)) {
                continue;
            }

            if ($data['phone'] && strpos($line, $data['phone']) !== false) {
                continue;
            }
